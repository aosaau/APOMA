...
export function CinematicStory({ mode, setMode }: CinematicStoryProps) {
  ...
  return (
    <div ref={containerRef} className="w-full flex flex-col relative bg-transparent">
      ...
      {/* Act III: Urgency */}
      <section className="relative w-full border-t border-current/10 mt-16" id="act-3">
         <SectionIntro 
          mode={mode}
          title={mode === 'physical' ? "The environment<br/>is changing." : "The infrastructure<br/>is ready."}
          description={mode === 'physical' ? "Whether we like it or not. Three forces are coming that make the next two years the most consequential in Andamooka's modern history." : "For the first time, every stone can carry proof of where it came from. Matrix opal will change from a speculative purchase into a verifiable asset.."}
        />
      ...
      <section className="relative w-full border-t border-current/10 mt-16" id="act-5">
        <SectionIntro 
          ...
          description="Replacing subjective human grading with edge-computed neural networks and spectral rarity scores. Absolute certainty at scale."
        />
      ...
      {mode === 'digital' && (
        <>
          ...
          <section className="relative w-full border-t border-current/10 mt-16" id="act-4">
             <SectionIntro 
              mode={mode}
          ...
          <section className="relative w-full border-t border-current/10 mt-16" id="act-8">
            <SectionIntro 
              mode={mode}
          ...
          <motion.div 
            ...
            className={cn(
              "w-full min-h-screen flex flex-col items-center justify-center text-center px-6 border-t border-current/10 relative overflow-hidden mt-16",
